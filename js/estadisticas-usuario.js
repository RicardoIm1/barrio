...
(function () {
    ...
    function instalarEstilos() {
        ...
        style.textContent = `
            .estadisticas-usuario-footer {
                width: 100%;
                margin: 1rem 0 0;
                padding: 1rem 0 0;
                border-top: 1px solid rgba(255,255,255,.12);
            }
            .estadisticas-usuario-footer .eu-cabecera {
            ...
            .estadisticas-usuario-footer .eu-grid {
                display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:.65rem;margin-bottom:1rem;
            }
            .estadisticas-usuario-footer .eu-card {
                padding:.8rem;border-radius:.7rem;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.1);
            }
            ...
            @media (max-width:600px) {
                .estadisticas-usuario-footer { padding-top:.8rem; }
                .estadisticas-usuario-footer .eu-grid { grid-template-columns:repeat(2,1fr); }
                .estadisticas-usuario-footer .eu-value { font-size:1.1rem; }
            }
        `;
    ...
    function dibujarGrafica(avisos) {
        ...
        for(let i=0;i<=4;i++){const y=pad.t+ch-ch*i/4;ctx.strokeStyle='rgba(255,255,255,.12)';ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke();ctx.fillStyle='rgba(255,255,255,.65)';ctx.fillText(String(Math.round(max*i/4)),pad.l-6,y);}
    ...
    async function cargar() {
        ...
        if(!footer)return;
        crearContenedor(usuario, footer);
        try {
            ...
            const {data:avisos,error}=await client.from('avisos').select('id,titulo,status,created_at,vistas').eq('created_by',usuario.id).neq('status','eliminado').order('created_at',{ascending:false});
