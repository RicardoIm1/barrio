...
(function () {
    ...
    function instalarEstilos() {
        ...
        style.textContent = `
            ...
            #estadisticas-usuario .eu-cabecera {
            ...
            #estadisticas-usuario .eu-grid {
                display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:.8rem;margin-bottom:1.25rem;
            }
            #estadisticas-usuario .eu-card {
                padding:1rem;border-radius:.8rem;background:#f8fafc;border:1px solid #e5e7eb;
            }
            ...
            @media (max-width:600px) {
                #estadisticas-usuario { padding:1rem; }
                #estadisticas-usuario .eu-value { font-size:1.3rem; }
            }
        `;
    ...
    function crearContenedor(usuario) {
        ...
        section.innerHTML = `
            <div class="eu-cabecera">
                <div>
                    <h2>Mis estadísticas</h2>
                    <p class="eu-sub">Rendimiento de los avisos publicados por ${escapar(usuario.nombre || 'ti')}</p>
                </div>
            </div>
            <div class="eu-grid">
            ...
            <div class="eu-panel">
                <h3>Publicaciones de los últimos 30 días</h3>
                <div class="eu-chart-wrap"><canvas id="eu-chart"></canvas></div>
            </div>
            <div class="eu-panel">
                <h3>Rendimiento por aviso</h3>
                <div class="eu-tabla-wrap">
                    <table>
                        <thead><tr><th>Aviso</th><th>Estado</th><th class="eu-num">Vistas</th><th class="eu-num">👍</th><th class="eu-num">👎</th><th class="eu-num">Interacciones</th></tr></thead>
                        <tbody id="eu-tabla-cuerpo"><tr><td colspan="6" class="eu-vacio">Cargando estadísticas...</td></tr></tbody>
                    </table>
                </div>
            </div>`;
    ...
    function dibujarGrafica(avisos) {
        ...
        const w = Math.max(320, Math.floor(rect.width || 700));
        const h = 190;
        ...
        avisos.forEach(a => {
            const k = fechaClave(a.created_at);
            if (k) conteo.set(k, (conteo.get(k) || 0) + 1);
        });
        const valores = dias.map(d => conteo.get(fechaClave(d)) || 0);
        ...
        for (let i = 0; i <= 4; i++) {
            ...
            ctx.fillText(String(Math.round(max * i / 4)), pad.l - 6, y);
        }

        const puntos = valores.map((v, i) => ({
            x: pad.l + (dias.length === 1 ? cw / 2 : cw * i / (dias.length - 1)),
            y: pad.t + ch - (v / max) * ch
        }));
        ...
        puntos.forEach(p => { ctx.beginPath(); ctx.arc(p.x, p.y, 3, 0, Math.PI * 2); ctx.fill(); });
        ...
        dias.forEach((d, i) => {
            if (i % 5 === 0 || i === dias.length - 1) {
                const x = pad.l + cw * i / (dias.length - 1);
                ctx.fillText(d.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit' }), x, h - pad.b + 8);
            }
        });
    }
    ...
    async function cargar() {
        ...
        if (!esUsuarioNormal(usuario) || !usuario.id) return;

        instalarEstilos();
        crearContenedor(usuario);

        try {
            const client = await (typeof window.getSupabaseClient === 'function'
                ? window.getSupabaseClient()
                : window.supabaseClient);
            if (!client) throw new Error('Cliente Supabase no disponible');

            const { data: avisos, error } = await client
                .from('avisos')
                .select('id,titulo,status,created_at,vistas,visitas')
            ...
            let positivos = 0;
            let negativos = 0;
            const votosPorAviso = new Map();

            if (lista.length) {
                const ids = lista.map(a => a.id);
                const { data: votos, error: errorVotos } = await client
                    .from('votos')
                    .select('aviso_id,tipo')
                    .in('aviso_id', ids);
                if (!errorVotos) {
                    (votos || []).forEach(v => {
                        const actual = votosPorAviso.get(v.aviso_id) || { positivos: 0, negativos: 0 };
                        if (v.tipo === 'positivo') { actual.positivos++; positivos++; }
                        if (v.tipo === 'negativo') { actual.negativos++; negativos++; }
                        votosPorAviso.set(v.aviso_id, actual);
                    });
                } else {
                    console.warn('No se pudieron cargar los votos de los avisos propios:', errorVotos);
                }
            }

            const vistas = lista.reduce((sum, a) => sum + numero(a.vistas ?? a.visitas), 0);
            const activos = lista.filter(a => a.status === 'activo').length;
            const interacciones = positivos + negativos;

            document.getElementById('eu-total-avisos').textContent = formatoNumero(lista.length);
            document.getElementById('eu-activos').textContent = formatoNumero(activos);
            document.getElementById('eu-vistas').textContent = formatoNumero(vistas);
            document.getElementById('eu-positivos').textContent = formatoNumero(positivos);
            document.getElementById('eu-negativos').textContent = formatoNumero(negativos);
            document.getElementById('eu-interacciones').textContent = formatoNumero(interacciones);

            dibujarGrafica(lista);

            const tbody = document.getElementById('eu-tabla-cuerpo');
            if (!lista.length) {
                tbody.innerHTML = '<tr><td colspan="6" class="eu-vacio">Todavía no tienes avisos publicados.</td></tr>';
                return;
            }

            tbody.innerHTML = lista.map(a => {
                const v = votosPorAviso.get(a.id) || { positivos: 0, negativos: 0 };
                const vistasAviso = numero(a.vistas ?? a.visitas);
                const inter = v.positivos + v.negativos;
                const estado = a.status || 'pendiente';
                const textoEstado = estado === 'activo' ? 'Activo' : estado === 'rechazado' ? 'Rechazado' : 'Pendiente';
                return `<tr>
                    <td><strong>${escapar(a.titulo || 'Sin título')}</strong></td>
                    <td><span class="eu-estado ${estado}">${textoEstado}</span></td>
                    <td class="eu-num">${formatoNumero(vistasAviso)}</td>
                    <td class="eu-num">${formatoNumero(v.positivos)}</td>
                    <td class="eu-num">${formatoNumero(v.negativos)}</td>
                    <td class="eu-num">${formatoNumero(inter)}</td>
                </tr>`;
            }).join('');
        } catch (error) {
            console.error('❌ Estadísticas personales:', error);
            const tbody = document.getElementById('eu-tabla-cuerpo');
            if (tbody) tbody.innerHTML = `<tr><td colspan="6" class="eu-vacio">No se pudieron cargar las estadísticas.</td></tr>`;
        }
    }
    ...
    function iniciar() {
        ...
        const usuario = obtenerUsuario();
        if (!esUsuarioNormal(usuario)) return;
